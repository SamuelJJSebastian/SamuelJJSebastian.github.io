...
int closestNode(Vec2 point){
  int shortest = 0;
  for (int i = 0; i < numNodes; i++){
    if (nodePos[i].distanceTo(point) < nodePos[shortest].distanceTo(point)){
      shortest = i;
    }
  }
  return shortest;
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  boolean xTrue = false;
  boolean yTrue = false;
  if (pointPos.x < boxTopLeft.x + boxW && pointPos.x > boxTopLeft.x) xTrue = true;
  if (pointPos.y < boxTopLeft.y + boxH && pointPos.y > boxTopLeft.y) yTrue = true;
  if (xTrue && yTrue){
    return true;
  }
  else return false;
...
void connectNeighbors(){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      hitInfo nodeHitCheck = rayBoxIntersect(boxTopLeft, boxW, boxH, nodePos[i], dir, distBetween);
      if (!nodeHitCheck.hit && !circleListCheck.hit && distBetween < 200){
        neighbors[i].add(j);
      }
    }
  }
