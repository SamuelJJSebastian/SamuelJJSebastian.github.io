...
void update(float dt){
  ...
  if (!(pos.distanceTo(mousePos) < 2)) pos.add(vel);
}

void draw(){
  update(1/frameRate);
  background(255,255,255); //White background DONE
  stroke(0,0,0);
  fill(255,0,0);
  circle(pos.x, pos.y,r*2); //Question: Why is it radius*2 ? To make the circle bigger
}
