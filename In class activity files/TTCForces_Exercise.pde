...
void setup(){
  ...
  agentPos[3] = new Vec2(480, 200);
  agentPos[4] = new Vec2(80, 310);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(500, 300);
  goalPos[4] = new Vec2(600, 600);
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  ...
  Vec2 velCollision = vel1.minus(vel2);
  float ttc = rayCircleIntersectTime(pos2, totalradius, pos1, velCollision);
  return ttc;
}
...
Vec2 computeAgentForces(int id){
  ...
  Vec2 goalForce = (goal_vel.minus(agentVel[id]));
  ...
  for (int i = 0; i < numAgents; i++){
    ...
    Vec2 futurePos_j = agentPos[i].plus(agentVel[i].times(ttc));
    ...
    if (agentPos[id].minus(goalPos[id]) == new Vec2(0,0)){
